...
NCYC_DB="${DIR}/resources/NCyc_100_ddb.dmnd"
NCYC_IDMAP="${DIR}/resources/ncyc_id2genemap.txt"
NSLOTS=12
OUTPUT_DIR="${DIR}/tmp"
SAMPLE_NAME=test
EVALUE=1e-5
...
}'  "${NCYC_IDMAP}" "${OUTPUT_DIR}/${SAMPLE_NAME}.blout" > \
    "${OUTPUT_DIR}/${SAMPLE_NAME}_ncy_annot.tsv"

if [[ $? -ne "0" ]]; then
  echo "awk format ncyc annotation failed"
  exit 1
fi  
