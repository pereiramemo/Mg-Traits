...
show_usage(){
  cat <<EOF
Usage: ./module8_hyd_mg_traits.sh <options>
...
-E 1 \
--cpu "${NSLOTS}" \
"${HYD_HMM}" "${INPUT_FILE}" > ${OUTPUT_DIR}/"${SAMPLE_NAME}.hout"
...
awk -v s="${SAMPLE_NAME}" -v  e="${EVALUE}" -v OFS="\t" '{ 
  ...
  for (i in array_annot) { 
   ...
   "${OUTPUT_DIR}/${SAMPLE_NAME}_hyd_annot.tsv"

if [[ $? -ne "0" ]]; then
  echo "awk format cant-hyd annotation failed"
  exit 1
fi  
...
"${OUTPUT_DIR}/${SAMPLE_NAME}_hyd_stats.tsv"

if [[ $? -ne "0" ]]; then
  echo "awk compute cant-hyd diversity failed"
  exit 1
fi  
